...
export default function ForgotPasswordPage() {
  ...
  if (sent) {
    return (
      <div className="text-center">
        ...
        <h1 className="mt-6 font-display text-3xl font-bold">Xat yuborildi</h1>
        <p className="mt-3 text-sm text-midnight-300">
          <span className="text-brass-300">{email}</span> manziliga parolni tiklash havolasi yuborildi.
        </p>
  ...
  return (
    <div>
      <Link
        href="/login"
        className="inline-flex items-center gap-1.5 text-xs text-midnight-300 hover:text-brass-300 mb-6"
      >
      ...
      <form onSubmit={onSubmit} className="mt-8 space-y-4">
        <div>
          <label className="block text-xs font-medium text-midnight-200 mb-1.5">Email</label>
          <div className="relative">
            <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-midnight-400" />
            <input
              type="email"
              value={email}
